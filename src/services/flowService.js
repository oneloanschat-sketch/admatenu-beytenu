...
const processMessage = async (phoneNumber, messageBody) => {
    ...
    switch (step) {
        ...
        case STEPS.GET_NAME:
            const procName = await aiService.processStep('GET_NAME', messageBody, session.data, lang);
            if (!procName.isValid) {
        ...
        case STEPS.LISTENING:
            ...
            const procList = await aiService.processStep('LISTENING', messageBody, session.data, lang);
        ...
        case STEPS.QUALIFICATION:
            const procQual = await aiService.processStep('QUALIFICATION', messageBody, session.data, lang);
        ...
        case STEPS.DATA_COLLECTION_CITY:
            const procCity = await aiService.processStep('DATA_COLLECTION_CITY', messageBody, session.data, lang);
            if (!procCity.isValid) {
        ...
        case STEPS.DATA_COLLECTION_PURPOSE:
            const procPurpose = await aiService.processStep('DATA_COLLECTION_PURPOSE', messageBody, session.data, lang);
        ...
        case STEPS.PROPERTY_OWNERSHIP:
            const procProp = await aiService.processStep('PROPERTY_OWNERSHIP', messageBody, session.data, lang);
            if (!procProp.isValid) {
        ...
        case STEPS.PROPERTY_DETAILS:
            const procDetails = await aiService.processStep('PROPERTY_DETAILS', messageBody, session.data, lang);
        ...
        case STEPS.RISK_CHECK:
            const procRisk = await aiService.processStep('RISK_CHECK', messageBody, session.data, lang);
            if (!procRisk.isValid) {
